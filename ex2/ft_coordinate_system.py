...
def calculate_distance(tuple):
    res = math.sqrt((tuple[0] - 0)**2 + (tuple[1] - 0)**2 + (tuple[2] - 0) **2)
    return round(res, 2)
...
def ft_coordinate_system():
    ...
    print(f"Distance between (0, 0, 0) and {tuple1}: {calculate_distance(tuple1)}\n")
    ...
    print(f"Distance between (0, 0, 0) and {tuple2}: {calculate_distance(tuple2)}\n")
    ...
    try:
        ...
        print(f'Error details - Type {type(e)}, Args: ("{e}",)\n')
