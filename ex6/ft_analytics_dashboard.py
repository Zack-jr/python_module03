...
def ft_analytics_dashboard():
    ...
    score_dict = {
        alice.player_name: alice.player_score,
        bob.player_name: bob.player_score,
        charlie.player_name: charlie.player_score,
        diana.player_name: diana.player_score,
    }
    player_score_categories = {
        c: sum(
            1
            for score in score_dict.values()
            if ("high" if score > 2000 else
                "medium" if score > 1000 else "low") == c
        )
        for c in categories
    }
    ...
    charlie.achievements = {"first_kill", "first_death"}
    diana.achievements = {
        "level_10",
        "level_20",
        "level_30",
    ...
    achievement_set = {
        achievement
        for player_achievements in achievement_list
        for achievement in player_achievements
    }
    active_regions = {
        region for region, status in region_dict.items() if status == "active"
    }
    ...
    average_score = (
        sum(player.player_score for player in old_list) / player_count
    )
    ...
    print(f"Top performer: {top_player.player_name} ({top_player.player_score}"
          f" points, {top_player.achievement_count} achievements)")


if __name__ == "__main__":
    ft_analytics_dashboard()

# - comprehensions are an efficient way to create new sequences in a single
# line
# - they can work with (ex:) lists, dicts, tuples and sets
